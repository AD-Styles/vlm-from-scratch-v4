...
게이트는 **raw 모델을 wrapper 없이** 평가합니다 — v3 에서 wrapper 가 raw 의 약함을 가렸기 때문입니다. bar 를 못 넘으면 배포는 자동 차단됩니다 (exit 1).
...
v4 는 Mini-LLaVA 시리즈 (v1 → v4) 의 **마지막 버전**입니다. v3 가 남긴 6개 숙제를 출발선에서 받아 다섯 개를 닫고 한 개(한국어 평가셋)를 절반까지 가져왔습니다 — 닫을 수 있는 건 v5 로 미루지 않고 이번에 닫았습니다.
