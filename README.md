...
> ✅ **학습·평가 완료 — 배포 게이트 GO.** v3 는 병목을 *"0.5B LLM"* 으로 지목했지만, 재검토 결과 절반은 **학습 데이터** 였습니다 (Stage 1 정렬 5K — LLaVA 의 ~1%). v4 는 **LLM 1.5B 확대 + 정렬 데이터 8배 확대 (5K→40K) + Stage 2 instruction 믹스 재설계** 를 함께 다루고, **배포 전 평가 게이트** 로 검증했습니다. 결과 (RTX 4060 Laptop 8GB 한 장, 모두 wrapper 없는 raw 모델): **VQAv2 36.67%→56.83%, POPE 50.00%→71.75%** — 게이트 5/5 통과. 상세는 아래 **📊 결과** 절 참고. v4 는 Mini-LLaVA 시리즈(v1→v4)의 **마지막 버전**입니다.
...
> Stage 2 믹스는 처음에 한 번 잘못 만들었습니다 (vqav2 단일 → terse-only 모델). 바로잡은 과정은 아래 **회고록 Step 6** 에 정직하게 기록했습니다. 풀 LLaVA 규모(558K/665K)는 노트북 한 장에서 비현실적입니다 — v4 의 현실 목표는 *"v3 보다 확실히 낫고 1.5B 급에선 준수"* 이지 GPT-4V 가 아닙니다.
...
> Stage 2 loss 가 v3(≈0.75)보다 높게 끝난 건 퇴행이 아닙니다 — 균형 믹스(긴 묘사·추론·한국어)가 vqav2 초단답보다 본질적으로 어렵습니다. 낮은 loss 는 terse 패턴 암기였을 뿐 (회고록 Step 6).
...
- **신호 단독 AUC** — CLIP-only 0.660, entropy-only **0.971**. LLM 첫 토큰 entropy 가 압도적으로 강한 신호입니다 (CLIP-ViT-B/32 는 만화 얼굴조차 "a person" 으로 무난히 매칭해 분포 밖임을 못 잡음). w_clip 을 0→1 로 스윕하면 AUC 가 **0.0(entropy 단독) 0.971 에서 1.0(CLIP 단독) 0.660 까지 단조 감소** — 재학습한 v4 모델은 자기 entropy 만으로 OOD 를 거의 가르고, CLIP 을 섞을수록 신호가 흐려집니다.
- **가중치가 모델 따라 움직였습니다** — v3 의 추측 0.6 → v4 초기 모델 보정 0.35 → v4 재학습 모델 보정 **0.0**. 모델이 좋아질수록 자기 불확실성(entropy)이 OOD 의 충분 신호가 됐다는 뜻입니다.
...
[`scripts/eval_gate.py`](scripts/eval_gate.py) 5개 기준 **5/5 통과** (재학습 모델, n=400).
...
### Step 3 — OOD: 임계값도 가중치도 미보정이었고, 모델이 좋아지자 답이 바뀌었다

N=2 → 100 케이스. v3 의 OOD 검출기는 임계값(0.5)과 결합 가중치(CLIP 0.6) 둘 다 미보정이었습니다. 100 케이스 ROC 로 w_clip 을 스윕하니, **재학습한 v4 모델에선 entropy 단독(w_clip 0.0)이 정점** 이었습니다 — entropy-only AUC 0.971 vs CLIP-only 0.660, 결합할수록 단조 감소. 처음엔 가중치 재조정을 다음 버전 숙제로 미루려 했지만 — **v4 가 시리즈의 마지막인 만큼 미루지 않고 끝냈습니다.** w_clip 0.0 + 임계값 0.4582 로 재보정 → 정확도 91%, 5-fold CV(AUC 0.969)로 과적합이 아님을 확인. 흥미로운 건 최적 가중치가 모델을 따라 움직였다는 점입니다 (v3 추측 0.6 → v4 초기 0.35 → v4 재학습 0.0) — 모델이 좋아질수록 자기 entropy 가 OOD 의 충분 신호가 됐습니다.
...
### Step 6 — Stage 2 데이터 믹스 실수, 그리고 재학습 (v4 의 가장 큰 시행착오)

Stage 1 정렬을 5K→40K(8배)로 늘린 것이 raw 성능 도약의 절반이라면, 나머지 절반은 **Stage 2 instruction 믹스를 두 번 만든 것** 입니다.

**1차 (실수).** `download_instruct_data.py` 의 기본값은 `localized_narratives + aokvqa + vqav2` 3종 균형 믹스인데, 첫 학습에서 `--configs vqav2` 로 **vqav2 단일** 만 받았습니다. vqav2 답변은 평균 5자 안팎의 초단답 — 그것만 보고 학습한 모델은 *무엇을 물어도 한 단어로* 답했습니다. "Describe this image" 에도 "Kitchen.", 한국어 장문 요청엔 환각. 그런데 **게이트는 이 모델을 5/5 로 통과시켰습니다** — VQAv2·POPE 둘 다 짧은-답 benchmark 라, terse-only 모델이 오히려 유리했던 것입니다.

**진단.** 배포된 데모를 직접 써 보고서야 문제를 발견했습니다. 처음엔 "4-bit 로 학습한 LoRA 를 fp32 로 서빙해서?" 를 의심해 `scripts/diag_deploy_gap.py` 로 4-bit vs fp32 답변을 직접 비교했지만 — 거의 동일, 가설 기각. 진짜 원인은 학습 데이터 믹스였습니다.

**2차 (수정).** localized_narratives 10K(긴 묘사) + aokvqa 6K(추론) + vqav2 18K + 한국어 12K = **46K 균형 믹스**로 다시 만들고, Stage 1 projector 를 재사용해 Stage 2 만 재학습했습니다 (7h22m). 재게이트는 회귀 없이 통과(VQAv2 56.8% / POPE 71.8%, 게이트가 측정하는 짧은-답 능력은 유지), 그리고 데모에서 "Describe" 가 온전한 문장으로 돌아왔습니다.

> **이게 v4 의 핵심 교훈입니다.** 정량 게이트(VQAv2/POPE)는 *측정하는 능력* 만 본증합니다 — 묘사·장문 같은 게이트 밖 능력이 통째로 빠져도 게이트는 GO 를 줍니다. 라이브 데모 정성 점검이 그 사각을 잡았습니다. 게이트와 데모 점검은 **상호 보완** 이지 어느 하나로 갈음되지 않습니다.

**작은 함정 둘 더.** 같은 다운로드 스크립트에서 (1) `the_cauldron` 이 질문 끝에 붙이는 *"Answer concisely."* 류 지시문 — 평가셋엔 없으므로 `strip_format_hint` 로 제거, (2) 물려받은 `--min-answer-len 4` 기본값이 `"yes"/"no"` 정답을 통째로 버려 POPE 신호를 무너뜨림 — 4→1 로 수정. 이 둘은 일찍 잡았습니다.

게이트도 한 번 헛디뎠습니다. 첫 측정 n=100 에서 POPE 가 표본 오차(SE≈4.8%p)와 스트림 순서 편향으로 흔들렸습니다 — **bar 를 손대지 않고** n=400 으로 늘려 안정화했습니다. "표본이 부족하면 게이트가 노이즈를 GO/NO-GO 로 잘못 굳힌다" 는 교훈을 남겼습니다.
